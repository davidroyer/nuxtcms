...
function nuxtModule(moduleOptions) {
  ...
  if (this.options.dev) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('get-posts', function (fn) {
        const content = {}
        cmsContent.contentTypes.forEach((contentType) => {
          const data = CONFIG.Api.read(`${contentType}/index.json`, 'json')
          // console.log('DATA FROM READ: ', data)

          content[contentType] = arrayFromObject(data)
        })

        // console.log('get-posts -- server side - content: ', content)
