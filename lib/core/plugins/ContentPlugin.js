...
class ContentPlugin {
  ...
  generateTagsFile(contentType) {
    // NOTE: I believe I can use `getCollectionArray`
    ...
    const dataObject = this.getApiFile(contentType)
    const dataArray = arrayFromObject(dataObject)
      .filter(contentItem => contentItem.tags.length > 0)
      .map((contentItem) => {
        delete contentItem.html
        return contentItem
    ...
    const apiData = groupBy(dataArray, ['tags'])
    this.config.Api.write(`${contentType.toLowerCase()}/tags/index.json`, apiData)
    return this
  }

  // generateSlimmArray(contentArray) {
  //   const articlesArray = arrayFromObject(articles).map((post) => {
  //     delete post.html
  //     return post
  //   })
  // }
  ...
  updateData(contentType, filepath) {
    ...
    if (!contentTypeExist) {
      console.log('Creating NEW API content type')
  ...
  writeApiFile(contentType, data) {
    this.config.Api.write(`${contentType.toLowerCase()}/index.json`, data)
    return this
  }
