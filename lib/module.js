...
function nuxtModule(moduleOptions) {
  ...
  DataWatcher
    .on('ready', () => {
      cmsData.handleData().handleRoutes()
    })
    ...
    .on('delete', (filepath, root) => {
      ...
      cmsData.deleteApiFile(filepath).handleRoutes()
    })

  ContentWatcher
    .on('ready', () => {
      cmsContent.generateContentApi().generateRoutesFile()
      initialWrite(CONFIG)
    })
    .on('change', (filepath, root, stat) => {
      cmsContent.handleUpdate(filepath).generateRoutesFile()
      handleItemUpdated(CONFIG, filepath)
    })
    .on('delete', (filepath, root) => {
      cmsContent.handleRemoval(filepath)
      // cmsContent.generateContentApi().generateRoutesFile()
