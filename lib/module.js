...
function nuxtModule(moduleOptions) {
  ...
  this.extendBuild((config, ctx) => {
    console.log('Extending Build')

    config.resolve.alias['@cmsApi'] = path.join(this.options.rootDir, 'example', options.apiDirectory)
  ...
  /**
   * NOTE: COULD POSSIBLY DO SOMETHING USEFUL HERE
   */
