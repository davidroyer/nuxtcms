...
class ContentPlugin {
  ...
  handleUpdate(filepath) {
    console.log('TCL: ContentPlugin -> handleUpdate -> filepath', filepath)
  ...
  updateApiFile(filepath) {
    const contentType = this.getFileName(filepath)
    this.config.Api.remove(`${contentType}`)
    return this
  }
  ...
  deleteApiFile(filepath) {
    const fileName = this.getFileName(filepath)
    this.config.Api.remove(`${fileName}`)
    return this
  }
}
