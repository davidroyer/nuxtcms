...
const { generateRoutes } = require('./helpers')
...
function nuxtModule(moduleOptions) {
  ...
  if (this.options.dev) {
    ...
    io.on('connection', (socket) => {
      ...
      //   fn(name + ' says ' + word);
      // });
      // const initialData = cmsContent.getCollectionArray('articles')
      // socket.emit('file-update', initialData)
      // console.log('articles array: ', data)
      fs.watch(`${CONFIG.apiDirectoryPath}`, { recursive: true }, (event, filename) => {
        console.log('fs.watch: event, filename', event, filename)
        const articlesArray = cmsContent.getCollectionArray('articles')
        console.log('TCL: articlesArray amount', articlesArray.length)

        // GET DATA
        io.emit('file-update', articlesArray, filename)
      })
    })
  }
  /********************************************************************************/
