...
function nuxtModule(moduleOptions) {
  ...
  /**
   * For local use to setup Jetpack references
   */
  const contentDirectoryPath = `${this.nuxt.options.srcDir}/${options.contentDirectory}`
  const apiDirectoryPath = `${this.nuxt.options.srcDir}/${options.apiDirectory}`

  /**
   * Creates watcher
   */
  const ContentWatcher = watcher.create(`${this.nuxt.options.srcDir}/${options.contentDirectory}`)

  /**
   * Sets up content and api directories for `fs-jetpack` to use for its CWD
   */
  const Content = jetpack.cwd(contentDirectoryPath)
  const Api = jetpack.cwd(apiDirectoryPath)
