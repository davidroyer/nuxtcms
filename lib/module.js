...
function nuxtModule(moduleOptions) {
  ...
  // if (this.options.dev) {
  // console.log('SHOULD SEE THIS ONLY IF IN DEV')
  ...
  this.nuxt.hook('generate:extendRoutes', (routes) => {
    ...
    const collectionRoutes = [...contentTypesArray, ...dataCollections].map(route => route.toLowerCase())
    console.log('TCL: collectionRoutes', collectionRoutes)
    ...
    dynamicRoutes.forEach((route) => {
      ...
      // }
      if (!collectionRoutes.includes(dir)) return

      const collectionObject = Api.read(`${dir}/index.json`, 'json')
      for (const slug in collectionObject) {
        routes.push({ route: `/${dir}/${slug}` })
      }
    })
  })
}
