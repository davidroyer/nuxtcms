...
export default (context, inject) => {
  ...
  routes.content.forEach(contentName => {
    const keyName = createKeys(contentName)
    const data = require(`@/_CMS/api/${contentName}`)
    
    context[keyName.forContext] = contentInjector(data)
    inject([keyName.forInject], contentInjector(data))
  });
  ...
  dataRoutes.forEach(fileName => {
    ...
    const keys = createKeys(fileName)
    
    context[keys.forContext] = data
    inject([keys.forInject], data)
  });
